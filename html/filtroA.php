        ...
        <div class="grid-container">
            ...
            try {
                ...
                if ($tipo == 'null') {
                    ...
                    $sql = "SELECT idtipo FROM tipo where tipo= :tipo";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute(array(':tipo' => $tipo));
                    $result = $stmt->fetchAll();
                    $onetrue = $result[0]['idtipo'];
                ...
                if ($cor == 'null') {
                    ...
                    $result = $stmt->fetchAll();
                    $fourtrue = $result[0]['idcor'];

                }


                $sql = "SELECT idprincipal FROM principal where ";
                if (isset($onetrue)) {
                    if ($sql == 'SELECT idprincipal FROM principal where ') {
                        $sql .= "tipo = $onetrue ";
                    } else {
                        $sql .= "and tipo = $onetrue ";
                    }
                } else {
                    $onetrue = 0;
                }
                if (isset($twotrue)) {
                    if ($sql == 'SELECT idprincipal FROM principal where ') {
                        $sql .= "categoria = $twotrue ";
                    } else {
                        $sql .= "and categoria = $twotrue ";
                    }
                } else {
                    $twotrue = 0;
                }
                if (isset($treetrue)) {
                    if ($sql == 'SELECT idprincipal FROM principal where ') {
                        $sql .= "formato = $treetrue ";
                    } else {
                        $sql .= "and formato = $treetrue ";
                    }
                } else {
                    $treetrue = 0;
                }
